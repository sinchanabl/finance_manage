...
switch ($selectedSort) {
    ...
    case 'none':
    default:
        $sortColumn = 'transaction_id DESC'; // Sort by ID in descending order (recently added)
        break;
}
...
$exp_fetched = mysqli_query($con, "SELECT t.transaction_id, t.tdate, t.amount, c.category_name 
                                   FROM transactions t 
                                   JOIN categories c ON t.category_id = c.category_id 
                                   WHERE t.user_id = '$userid' $orderByClause");
?>
<!DOCTYPE html>
<html lang="en">
                        ...
                        <table class="table table-hover table-bordered">
                            ...
                            <?php $count=1; while ($row = mysqli_fetch_array($exp_fetched)) { ?>
                                <tr>
                                    ...
                                    <td class="text-center"><?php echo $row['tdate']; ?></td>
                                    <td class="text-center"><?php echo $row['amount']; ?></td>
                                    <td class="text-center"><?php echo $row['category_name']; ?></td>
                                    <td class="text-center">
                                        <a href="add_expense.php?edit=<?php echo $row['transaction_id']; ?>" class="btn btn-primary btn-sm" style="border-radius:0%;">Edit</a>
                                    </td>
                                    <td class="text-center">
                                        <a href="add_expense.php?delete=<?php echo $row['transaction_id']; ?>" class="btn btn-danger btn-sm" style="border-radius:0%;">Delete</a>
                                    </td>
                                </tr>
                            <?php $count++; } ?>
                        </table>
